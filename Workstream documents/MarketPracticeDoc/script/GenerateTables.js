...
if(!endpoint)
  endpoint = '/parties/{Type}/{ID}/error';
if(!http_method)
  http_method = 'put';
if(!iso20022Path)
  iso20022Path = './Workstream documents/MarketPracticeDoc/script/acmt.024.001.04.xsd';
if(!outputFilename)
  outputFilename = './Workstream documents/MarketPracticeDoc/script/parties_error_PUT.md';
if(!templatePath)
  templatePath = './Workstream documents/MarketPracticeDoc/script/parties_error_PUT_template.md';
...
parser.parseString(iso20022Xml, (err, iso20022Doc) => {
  ...
  const extractRequiredFields = (schema, descriptionOverrides, fieldNameOverrides,  parentField = null, parentRequired = true, or_text="") => {
    if (schema.$ref) {
    ...
    required.forEach(field => {
      ...
      if (descriptionOverrides.hasOwnProperty(fullFieldName)) {
        description = descriptionOverrides[fullFieldName];
      } else if (description != "") {
        descriptionOverrides[fullFieldName] = description;
      }
      if (fieldNameOverrides.hasOwnProperty(fullFieldName)) {
        fieldName = fieldNameOverrides[fullFieldName];
      } else if (fieldName != "") {
        fieldNameOverrides[fullFieldName] = fieldName;
      }
      ...
      if (schema.properties && schema.properties[field]) {
        extractRequiredFields(schema.properties[field], descriptionOverrides, fieldNameOverrides,  fieldObj, true);
      }
    });
    }
    ...
    if (schema.allOf) {
      schema.allOf.forEach(subSchema => {
        extractRequiredFields(subSchema, descriptionOverrides, fieldNameOverrides, parentField, parentRequired,or_text);
      });
    }
    ...
    if (schema.oneOf) {
      ...
      schema.oneOf.forEach(subSchema => {
        ...
        Or_open = true;
        extractRequiredFields(subSchema, descriptionOverrides,fieldNameOverrides,  parentField, parentRequired, l_or_text);
      });
    }
  ...
  const extractOptionalFields = (schema, descriptionOverrides, fieldNameOverrides, parentField = null, parentRequired = true, or_text = "") => {
    if (schema.$ref) {
      schema = resolveRef(schema.$ref);
    }
    ...
    required.forEach(field => {
      ...
      if (schema.properties && schema.properties[field]) {
        extractOptionalFields(schema.properties[field], descriptionOverrides,fieldNameOverrides,  fieldObj, true);
      }
    });
    }
    ...
    if (schema.properties) {
      Object.keys(schema.properties).forEach(field => {
        if (!parentRequired || !required.includes(field)) {
          ...
          extractOptionalFields(schema.properties[field], descriptionOverrides, fieldNameOverrides, fieldObj, false);
        }
      });
    }
    ...
    if (schema.allOf) {
      schema.allOf.forEach(subSchema => {
        extractOptionalFields(subSchema, descriptionOverrides, fieldNameOverrides, parentField, parentRequired,or_text);
      });
    }
    ...
    if (schema.oneOf) {
      ...
      schema.oneOf.forEach(subSchema => {
        const l_or_text = Or_open ? "or}" : "{or";
        Or_open = true;
        extractOptionalFields(subSchema, descriptionOverrides, fieldNameOverrides, parentField, parentRequired, l_or_text);
      });
    }
...
// Load descriptionOverrides from a file
const descriptionOverridesFilePath = `${outputFilename}_descriptionOverrides.json`;
let descriptionOverrides = {};
if (fs.existsSync(descriptionOverridesFilePath)) {
  try {
    descriptionOverrides = JSON.parse(fs.readFileSync(descriptionOverridesFilePath, 'utf8'));
    console.log('Loaded descriptionOverrides from file:', descriptionOverridesFilePath);
  } catch (error) {
    console.error('Error loading descriptionOverrides file:', error.message);
  }
} else {
  console.warn('descriptionOverrides file not found. Using an empty object.');
}

// Load descriptionOverrides from a file
const fieldNameOverridesFilePath = `./${outputFilename}_fieldNameOverrides.json`;
let fieldNameOverrides = {};
if (fs.existsSync(fieldNameOverridesFilePath)) {
  try {
    fieldNameOverrides = JSON.parse(fs.readFileSync(fieldNameOverridesFilePath, 'utf8'));
    console.log('Loaded fieldNameOverrides from file:', fieldNameOverridesFilePath);
  } catch (error) {
    console.error('Error loading fieldNameOverrides file:', error.message);
  }
} else {
  console.warn('fieldNameOverrides file not found. Using an empty object.');
}


  let openAPISchema = endpointPath.requestBody.content['application/json'].schema;
  extractRequiredFields(openAPISchema, descriptionOverrides, fieldNameOverrides);
  extractOptionalFields(openAPISchema, descriptionOverrides, fieldNameOverrides);
    ...
    const startDepth = 2;
    const extractIsoFields = (element, descriptionOverrides, fieldNameOverrides, parentField = null, depth = 0, parentSchema = null) => {
      if (element['xs:element']) {
        element['xs:element'].forEach(el => {
          ...
          if (descriptionOverrides.hasOwnProperty(fullName)) {
            description = descriptionOverrides[fullName];
          }    
          if (fieldNameOverrides.hasOwnProperty(fullName)) {
            fieldName = fieldNameOverrides[fullName];
          }                
          ...
          if (el['xs:complexType']) {
            ...
            if (complexType) {
              extractIsoFields(complexType, descriptionOverrides, fieldNameOverrides, fieldObj, depth + 1);
            }
          }
        });
      }
      element['xs:choice']?.forEach(el => {
        extractIsoFields(el, descriptionOverrides, fieldNameOverrides, parentField, depth);
      });
      element['xs:sequence']?.forEach(el => {
        extractIsoFields(el, descriptionOverrides, fieldNameOverrides, parentField, depth);
      });
      if (element['xs:complexType']) {
        extractIsoFields(element['xs:complexType'], descriptionOverrides, fieldNameOverrides, parentField,depth);
      } 
    };
  
    if (iso20022Doc['xs:schema']) {
      extractIsoFields(iso20022Doc['xs:schema'], descriptionOverrides, fieldNameOverrides);
    } else {
...
<table>
  ...
  // Save updated descriptionOverrides to a new file
  const newDescriptionOverridesFilePath = `./${outputFilename}_descriptionOverrides_new.json`;
  try {
    fs.writeFileSync(newDescriptionOverridesFilePath, JSON.stringify(descriptionOverrides, null, 2), 'utf8');
    console.log('Saved updated descriptionOverrides to file:', newDescriptionOverridesFilePath);
  } catch (error) {
    console.error('Error saving updated descriptionOverrides file:', error.message);
  }
    // Save updated fieldNameOverrides to a new file
    const newFieldNameOverridesFilePath = `./${outputFilename}_fieldNameOverrides_new.json`;
  try {
    fs.writeFileSync(newFieldNameOverridesFilePath, JSON.stringify(fieldNameOverrides, null, 2), 'utf8');
    console.log('Saved updated fieldNameOverrides to file:', newFieldNameOverridesFilePath);
  } catch (error) {
    console.error('Error saving updated fieldNameOverrides file:', error.message);
  }
